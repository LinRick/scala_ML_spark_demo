...
object functions {
  ...
  def score = udf { sentence: Double =>
      var sentiment = new String
      if (sentence < 2.0 && sentence >= 0.0) sentiment = "neg"
      if (sentence > 2.0 && sentence <= 4.0) sentiment = "pos"
      if (sentence == 2.0) sentiment = "neu"
      if (sentence == -1.0) sentiment = "unk"
      sentiment
  }
}
